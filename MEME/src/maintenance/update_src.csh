...
#
# Version Information:
# 12/29/2006 BAC (1-D57N7): leave insert_meta_version blank for new SABs and
#                           remove_meta_version blank for old SABs.
# 12/28/2006 BAC (1-D53YL): fixed typo in get-oracle-pwd.pl call
...
while ($i <= $#argv)
 ...
 Options:
     <release_ver>:      Release version, e.g. 2012AA (should be the current AA version)
     <old_release_ver>:  Old release version, e.g. 2011AA (should be the previous AA version)
     <database>:         Database
     -v version:         Version information
     -h help:            On-line help
...
foreach rsource (MED MBD)
    ...
    -- No longer do this: remove_meta_version = '$old_release_ver',
    UPDATE sims_info
        SET
...
1|1|C|SEMANTIC_TYPE|Intellectual Product|SRC|R|Y|N|N|SRC_ATOM_ID|||3d9e88091cf4ebbab774e90c8f6d4052|
EOF

    echo "    Writing into relationships.src ... `/bin/date`"
    /bin/cat >! relationships.src <<EOF
1|S|V-$rsource|NT|version_of|1|SRC|SRC|R|Y|N|N|CODE_SOURCE|SRC|SRC_ATOM_ID||||
EOF
