...
#
# This script is used to redo action.
#
# Version Information
#
...
#
$ec = 0;
system ("$ENV{MEME_HOME}/bin/memerun.pl -view=false $prop $host $port gov.nih.nlm.meme.client.ActionClient $database redo_batch $transaction_id $work_id $force $authority");
if ($?) { $ec=1; }
...
sub PrintUsage {
    ...
    redo.pl [-prop=<file>] [-host=<host>] [-port=<port>] [-w <work_id>] [-f]
...
sub PrintHelp {
	&PrintUsage;
	print qq{
 This script is used to perform redo actions.
