/*****************************************************************************
*
* File:  $MEME_HOME/etc/sql/meme_tables.sql
...
*     1. Add any indexes to $MEME_HOME/meme_indexes.sql
*     2. Add the table to $MEME_HOME/meme_synonyms.sql
*     3. Call MEME_SYSTEM.register_table(table_name)
*     4. $MRD_HOME/etc/sql/meme_tables.sql
*     5. Add descriptions to table_documentation.pl
*
...
* 05/23/2005 3.40.0: Released
* 05/19/2005 3.39.1: Bug fix in sims_info (dup meta_year, plus missing / at
*                    head of file)
* 05/16/2005 3.39.0: Released, +src_obsolete_qa_results
* 01/14/2005 3.38.1: sims_info has longer fields and +whats_new
...
*            Released to URL /MEME/Data/meme_tables.sql
*
...
CREATE TABLE source_replacement (
	...
	sg_meme_data_type	VARCHAR2(10),
	sg_meme_id		NUMBER(12) DEFAULT 0,
	new_sg_meme_id		NUMBER(12) DEFAULT 0
)
PCTFREE 10 PCTUSED 80 MONITORING
STORAGE (INITIAL 50M);
...
-- This table holds obsolete qa count results from the
-- source tables.  A single QA_id represents a complete
-- set of data to compare against, and so can span
-- multiple sources.
--
-- When a source is updated, the counts are moved to this table
DROP TABLE src_obsolete_qa_results;
