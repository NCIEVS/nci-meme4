...
set CD=/bin/chdir
set COPY=/bin/cp
set update_dir="$MEDLINE_DIR/update";
...
#pushd $update_dir;
foreach f (`ls coc_headings_med*.dat|$SORT`)
echo "Processing $f";
set name=`echo $f | cut -d "_" -f3 |cut -d"." -f1`;  
$COPY `echo coc_headings_$name.dat` coc_headings.dat
$COPY `echo coc_subheadings_$name.dat` coc_subheadings.dat
$COPY `echo coc_headings_todelete.$name.dat` coc_headings_todelete.dat
