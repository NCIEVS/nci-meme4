...
set version = "4.0"
set version_date = "03/28/2008"
set version_auth = "DSS"
set usage="Usage: $0 <meta_dir> <net_dir> <mmsys_dir>"
...
if ($#argv == 1) then
 ...
 This script is used to build the configuration files used
 by MetamorphoSys.  It requires that the first parameter <meta_dir>
 is set to a directory containing validated MR*.RRF 
 files. The second parameter <net_dir> must be set to a directory
 containing the Semantic Network Files including SRDEF. This 
 script writes output to the <mmsys_dir>/config/<release version>
 directory.  The <mmsys_dir> is the users third parameter and
 <release version> is extricated from the release.dat file
 which must be located in the <mmsys_dir> before running the script.

 This script is run AFTER all of the files have
 been built.  It produces default metamorphosys properties
 files:  user.*.prop and umls.prop.  These, along with other
 files required by MetamorphoSys 
 will be copied to the <mmsys_dir>/config/<release version> directory.
...
$PATH_TO_PERL -ne 'chop; split /\|/; \
  if ($_[8] eq "SUBSET_MEMBER" || $_[8] eq "CV_MEMBER")  { \
     @fields = split(/~/, $_[10]); \
     next if ($fields[1] eq ""); \
     $atn = $fields[1]; \
     next if $map{"$atn$_[9]"}; \
...
if ($f == b) set name="Level 0 + SNOMEDCT_US"
if ($f == b) set description="Exclude all non-level 0 sources except SNOMEDCT_US"
if ($f == c) set name="SNOMEDCT_US"
if ($f == c) set description="Include only SNOMEDCT_US"
...
if ($f == "c") then
   echo "gov.nih.nlm.umls.mmsys.filter.SourceListFilter.selected_sources=SNOMEDCT_US|SNOMEDCT_US" >> $OUTPUT_DIR/user.$f.prop
else if ($f == "d") then
   echo "gov.nih.nlm.umls.mmsys.filter.SourceListFilter.selected_sources=AIR|AIR;AOD|AOD;BI|BI;CCPSS|CCPSS;COSTAR|COSTAR;CPTSP|CPT;CST|CST;DDB|DDB;DMDUMD|UMD;DSM3R|DSM3R;DXP|DXP;HLREL|HLREL;ICPC|ICPC;JABL|JABL;LCH|LCH;MCM|MCM;MTHMST|MTHMST;NCISEER|NCISEER;PCDS|PCDS;PPAC|PPAC;QMR|QMR;RAM|RAM;RCD|RCD;SNM|SNM;SNMI|SNMI;ULT|ULT;WHO|WHO;ICPCBAQ|ICPC;ICPCDAN|ICPC;ICPCDUT|ICPC;ICPCFIN|ICPC;ICPCFRE|ICPC;ICPCGER|ICPC;ICPCHEB|ICPC;ICPCHUN|ICPC;ICPCITA|ICPC;ICPCNOR|ICPC;ICPCPOR|ICPC;ICPCSPA|ICPC;ICPCSWE|ICPC;MTHFDA|MTHFDA;MTHMSTFRE|MTHMST;MTHMSTITA|MTHMST;RCDAE|RCD;RCDSA|RCD;RCDSY|RCD;WHOFRE|WHO;WHOGER|WHO;WHOPOR|WHO;WHOSPA|WHO" >> $OUTPUT_DIR/user.$f.prop
else
    $PATH_TO_PERL -e '\
	...
	while ($line = <SAB>) { \
	  if ($ARGV[0] eq "b" && ($line =~ /SNOMEDCT_US/ || $line =~ /MTHSCT/)) { next; } \
...
foreach cui (`/bin/cut -d\| -f 1 $META_RELEASE/cvfmrsat.dat | /bin/sort -u`)
    ...
    set append_value=($cui"|"$name"|"$description"|"$code"|"$includeObsolete"|"$cascade";")
