 ...
 * source concept identifier and that {@link Source} is listed in <code>ic_single</code>.
 *
 * @author MEME Group
 */
public class MGV_SCUI extends AbstractUnaryDataMergeMoveInhibitor {
  ...
  /**
   * Instantiates a {@link MGV_SCUI} check.
   */
  public MGV_SCUI() {
    super();
    setName("MGV_SCUI");
      ...
      if (l_source_atoms[i].isReleasable()) {
        for (int j = 0; j < target_atoms.length; j++) {
          if (target_atoms[j].isReleasable() &&
              ...
              target_atoms[j].getSourceConceptIdentifier()!= null &&
              !target_atoms[j].getSourceConceptIdentifier().equals(l_source_atoms[i].getSourceConceptIdentifier())) {
            return true;
          }
        }
      }
