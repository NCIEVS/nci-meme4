 ...
 * @author MEME Group
 */
public class MGV_PN extends AbstractMergeInhibitor {
  ...
  public boolean validate(Concept source, Concept target) {
    ...
    for (int i = 0; i < source_atoms.length; i++) {
      if (source_atoms[i].getTermgroup().getTermType().equals("PN") &&
  ...
  public static void main(String[] args) {
    ...
    MEMEToolkit.trace("-------------------------------------------------------");
    MEMEToolkit.trace("Starting test of MGV_PN ..." + new Date());
    MEMEToolkit.trace("-------------------------------------------------------");

    boolean failed = false;

    String test_message = "Testing MGV_PN- validate():";
    String test_result = null;

    // Create a MGV_PN object to work with
    MGV_PN mgv_pn = new MGV_PN();
    ...
    if (!mgv_pn.validate(source, target)) {
      test_result += " PASSED: ";
    } else {
      test_result += " FAILED: ";
      failed = true;
    ...
    MEMEToolkit.trace(test_message + test_result +
        "Merging concepts where both contain releasable MTH/PN atoms.");
    ...
    MEMEToolkit.trace(test_message + test_result +
        "One of the merging concepts does not contain releasable MTH/PN atoms.");
    ...
    MEMEToolkit.trace(test_message + test_result +
                      "One of the merging concepts does not contain MTH atoms.");
    ...
    MEMEToolkit.trace(test_message + test_result +
        "One of the merging concepts does not contain PN termtype.");
    ...
    MEMEToolkit.trace("-------------------------------------------------------");
    MEMEToolkit.trace("Finished test of MGV_PN ..." + new Date());
    MEMEToolkit.trace("-------------------------------------------------------");
