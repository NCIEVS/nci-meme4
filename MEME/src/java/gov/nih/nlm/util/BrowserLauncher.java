 ...
 * Object:      BrowserLauncher.java
 * 
 * Changes
 *   01/06/2006 BAC (1-72TDI): support firefox.
 ***********************************************************************/
package gov.nih.nlm.util;
...
/**
 *
 * Check for updates: http://sourceforge.net/projects/browserlauncher/
 *
...
public class BrowserLauncher {
  ...
  private static Object locateBrowser() {
    ...
    switch (jvm) {
      ...
      case OTHER:
      default:
        browser = getFirefoxMozillaOrNetscape();
        break;
    }
    return browser;
  }
  ...
  /**
   * Determines whether to use netscape or mozilla.
   * @return command for "netscape" or "mozilla"
   */
  private static String getFirefoxMozillaOrNetscape() {
    try {

      //
      // Find out if mozilla or netscape is running
      //
      ...
      boolean f_firefox = false;
      boolean f_mozilla = false;
      while ( (line = in.readLine()) != null) {
        if (line.indexOf("firefox") != -1) {
          p.waitFor();
          f_firefox = true;
        }
      ...
      if (f_firefox) return "firefox";
      if (f_mozilla) return "mozilla";

      //
      // OK, at this point, neither is running, see if mozilla is in the path
      //
      cmd = new String[] {
          "which", "firefox"};
      ...
      //
      // Something went wrong, just try firefox
      //
      return "firefox";
    }
  }
  ...
  public static void main(String[] args) {
    try {
      BrowserLauncher.openURL("http://meow.nlm.nih.gov");
    } catch (IOException e) {
      e.printStackTrace();
