 ...
 * Object:  TestReportFrame
 *  Version:  Copied the file from client directory (1-EO35F)
 *****************************************************************************/

import gov.nih.nlm.meme.client.FinderClient;
import gov.nih.nlm.meme.client.ReportsClient;
import gov.nih.nlm.meme.client.TestFinderFrame;
...
import gov.nih.nlm.util.OnePagePrintable;
import gov.nih.nlm.util.PrintPreviewAction;
...
public class TestReportFrame extends JFrame implements GlassPaneListener {
   ...
   */
  public TestReportFrame() throws Exception {
    this("editing-db");
  }
   ...
   * @throws Exception if clients cannot be initialized
   */
  public TestReportFrame(String mid_service) throws Exception {
    ...
    Action ppa = new PrintPreviewAction(null, pcra) {
	...
	public void actionPerformed(ActionEvent ae) {
        ...
        editor.setText(
            ...
            SystemToolkit.removeLinks(getText()) + "</body></html>");
        setPrintable(new OnePagePrintable(editor, getTitle()));
        setTitle(getTitle());
        super.actionPerformed(ae);
      }
    };
    JButton print_p = new JButton(ppa);
    btn_panel.add(print_p);
    ...
    Thread thread = new Thread(
      ...
      public void run() {
        ...
        try {
          ...
          TestReportFrame myReport = JekyllKit.getReportFrame(true);
          myReport.showMessage("<html><head><style> body {font-size: "
                  + 12 + "pt; }</style></head><body>"
                  + rpt + "</body></html>");
          myReport.setTitle("Concept Report for: " + id);
          glass.setVisible(false);
        } catch (Exception e) {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      SwingToolkit.setProperty("meme.view", "true");
