...
sub exists {
  ...
  return 0 unless $oracleDBH->tableExists($uristable);

  my($sql) = "select max(timestamp) from $uristable where " .
    join(' and ',
	 "version=" . $oracleDBH->quote($version),
	 "mrtable=" . $oracleDBH->quote($filename),
	 "colnames=" . $oracleDBH->quote(join(',', @$cols)));
  return 1 if $oracleDBH->selectFirstAsScalar($sql);
  $oracleDBH->disconnect;
  return 0;
}
