...
public class MGV_H1Test extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		ic.setIsActive(true);
		ic.setIsFatal(true);
		ic.setShortDescription("Short description of MGV_H1");
		ic.setDescription("Description of MGV_H1.");
	...
	public void testEmptyTargetConcept() {
		System.out.println("MGV_H1Test#testEmptyTargetConcept()");
		createSourceConcept();
	...
	public void testEmptySourceConcept() {
		System.out.println("MGV_H1Test#testEmptySourceConcept()");
	...
	public void testViolationFound() {
		System.out.println("MGV_H1Test#testViolationFound()");
	...
	public void testViolationNotFoundWrongCode() {
		System.out.println("MGV_H1Test#testViolationNotFoundWrongCode()");
		createSourceConceptSetWrongCode();
		createTargetConceptSetWrongCode();
		assertTrue("Violation expected.", !ic.validate(source, target));
		assertTrue("Violation expected.", !ic.validate(source, target, atoms));
	}

	public void testViolationNotFoundNonSetSAB() {
		System.out.println("MGV_H1Test#testViolationNotFoundSetSAB()");
	...
	public void testViolationNotFoundNonReleasable() {
		System.out.println("MGV_H1Test#testViolationNotFoundNonReleasable()");
	...
	public void testViolationNotFoundNonCurrent() {
		System.out.println("MGV_H1Test#testViolationNotFoundNonCurrent()");
	...
	private void createTargetConceptSetCode() {
		...
		Atom atom = createAtom();
		atom.setConcept(target);
		atom.setCode(new Code("D55555"));
		target.addAtom(atom);
	}

	private void createSourceConceptSetWrongCode() {
		source = new Concept.Default();
		Atom atom = createAtom();
		atom.setConcept(source);
		atom.setCode(new Code("X12345"));
		source.addAtom(atom);
		atoms = new Atom[] { atom };
	}

	private void createTargetConceptSetWrongCode() {
		target = new Concept.Default();
		Atom atom = createAtom();
		atom.setConcept(target);
		atom.setCode(new Code("X55555"));
		target.addAtom(atom);
	}
