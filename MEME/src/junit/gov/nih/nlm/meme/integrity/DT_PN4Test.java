...
public class DT_PN4Test extends TestCase {

	private Concept source = null;
	private DT_PN4 ic = null;
	...
	protected void setUp() throws Exception {
		...
		ic.setIsActive(true);
		ic.setIsFatal(true);
		ic.setShortDescription("Short description of DT_PN4");
		ic.setDescription("Description of DT_PN4.");
	...
	public void testViolationNotFoundNoAtoms() {
		System.out.println("DT_PN4Test#testViolationNotFoundNoAtoms()");
		createConcept();
		assertTrue("No violation expected. No atoms attached to concept.", !ic.validate(source));
	}

	public void testViolationFound() {
		System.out.println("DT_PN4Test#testViolationFound()");
		createSourceConcept();
		assertTrue("Violation expected.", ic.validate(source));
	}

	public void testViolationNotFoundSetTTY() {
		System.out.println("DT_PN4Test#testViolationNotFoundSetTTY()");
		createSourceConceptSetTTY();
		assertTrue("No violation expected.", !ic.validate(source));
	}

	public void testViolationNotFoundSetTBR() {
		System.out.println("DT_PN4Test#testViolationNotFoundSetTBR()");
		createSourceConceptSetTBR();
		assertTrue("No violation expected.", !ic.validate(source));
	}

	public void testViolationNotFoundSetAmbiguous() {
		System.out.println("DT_PN4Test#testViolationNotFoundSetAmbiguous()");
		createSourceConceptSetAmbiguous();
		assertTrue("No violation expected.", !ic.validate(source));
	}
	...
	private void createSourceConceptSetAmbiguous() {
		source = new Concept.Default();
		Atom atom = createAtom();
