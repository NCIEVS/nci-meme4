...
public class DT_PN2Test extends TestCase {

	private Concept source = null;
	private DT_PN2 ic = null;
	...
	protected void setUp() throws Exception {
		...
		ic.setIsActive(true);
		ic.setIsFatal(true);
		ic.setShortDescription("Short description of DT_PN2");
		ic.setDescription("Description of DT_PN2.");
	...
	public void testViolationNotFoundNoAtoms() {
		System.out.println("DT_PN2Test#testViolationNotFoundNoAtoms()");
		createConcept();
		assertTrue("No violation expected. No atoms attached to concept.", !ic.validate(source));
	}

	public void testViolationFound() {
		System.out.println("DT_PN2Test#testViolationFound()");
		createSourceConcept();
		assertTrue("Violation expected.", ic.validate(source));
	}

	public void testViolationNotFoundSetSAB() {
		System.out.println("DT_PN2Test#testViolationNotFoundSetSAB()");
		createSourceConceptSetSAB();
		assertTrue("No violation expected.", !ic.validate(source));
	}

	public void testViolationNotFoundSetTTY() {
		System.out.println("DT_PN2Test#testViolationNotFoundSetTTY()");
		createSourceConceptSetTTY();
		assertTrue("No violation expected.", !ic.validate(source));
	}

	public void testViolationNotFoundSetTBR() {
		System.out.println("DT_PN2Test#testViolationNotFoundSetTBR()");
		createSourceConceptSetTBR();
		assertTrue("No violation expected.", !ic.validate(source));
	}

	public void testViolationNotFoundAddSingleAtom() {
		System.out.println("DT_PN2Test#testViolationNotFoundAddSingleAtom()");
		createSourceConceptAddSingleAtom();
		assertTrue("No violation expected.", !ic.validate(source));
	}
	...
	private void createSourceConceptAddSingleAtom() {
		source = new Concept.Default();
		Atom atom = createAtom();
		atom.setConcept(source);
		source.addAtom(atom);
