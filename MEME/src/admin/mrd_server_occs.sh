...
ENV_FILE=/umls_prod/umls_apps/env/etc/env.prop.mrdp.$hostname
export ENV_HOME
export ENV_FILE
. $ENV_HOME/bin/env.sh
PROPERTIES_FILE=$MEME_HOME/etc/mrd.prop.$hostname
date=`/bin/date +%Y%m%d`;
time=`/bin/date +%H%M%S`;
...
start_mrd() {
	...
	$MRD_HOME/bin/memerun.pl  -headless -port $PORT -prop $PROPERTIES_FILE -mid $MS -mrd $MS gov.nih.nlm.mrd.server.MRDApplicationServer 1>$MRD_HOME/log/`/bin/date +%Y%m%d`.$$.log 2>&1 &
}
