...
<%
	...
	if((code & StageStatus.FINISHED) == StageStatus.FINISHED) {
	  if(stagestatus[j].getName().equals("validate")) {
    		status = "<td>Finished</td><td><font size=-1><A target=\"_blank\" href=\"controller?state=TargetDetails&release=" + release_info.getName() + "&target=" + request.getParameter("target") + "&detail=View QAResult\" >View QAResult</A></font>&nbsp;|&nbsp;" +
    			  ...
    			  "<font size=-1><A target=\"_blank\" href=\"controller?state=TargetDetails&release=" + release_info.getName() + "&target=" + request.getParameter("target") + "&detail=View QAReport&type=html\">View QAReport</A></font>&nbsp;</td>";
	  } else {
    		status = "<td>Finished</td><td><font size=-1><a target=\"_blank\" href=\"controller?state=TargetDetails&release=" + release_info.getName() + "&target=" +request.getParameter("target") + "&stage=" + stagestatus[j].getName() +"&detail=View Log\" >View Log</a></font></td>";
	  }
	}
