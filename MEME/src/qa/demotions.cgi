...
#
# Description:
#    This CGI script investigates demotions attached to
...
%states = (
	     "INDEX" => 
	         ["PrintHeader","PrintINDEX","PrintFooter","Demotions Detective","Demotions Detective"],
	     "REPORT" => 
	         ["PrintHeader","PrintREPORT","PrintFooter", "Demotions Report","Report of Demotions for: $concept_id"],
	     "" => 
...
sub PrintINDEX {
    ...
    $databases{"testmisc-db"} = &meme_utils::midsvcs("testmisc-db");

    print qq{
This tool is used to investigate the demotions that a particular concept is involved in.  Please select the database you want and enter the concept ID.  Click <i>Submit</i> to generate the report.
...
#
# This procedure prints a report of the demotions connected
...
# open connection
  ...
  while (($relationship_id, $atom_id_1, $atom_id_2) = $sh->fetchrow_array) {
    ...
    unless ($row_count) {
      print qq{
<i>Following are the demotions affecting this concept and (hopefully) an explanation of the circumstances behind each demotion</i>.<p>
};
     }
...
Relationship ID: $relationship_id</pre>
    ...
    if ($authority) {
      if ($work_id) {
	$work_id_clause = " AND (work_id = $work_id OR work_id = 0) ";
      }
      $sh2 = $dbh->prepare(qq{
	...
	$vv_info = qq{The demotion is not the result of integrity violations, this
		      was created by a recipe step ($merge_set_info) that explicitly adds demotions.};
      } else {
  ...
  unless ($row_count) {
    print qq{
<span id="red">There are no demotions connected to concept $concept_id</span>
     }
   }
...
sub PrintHelp {
    print qq{
 This script is a CGI script used determine the cause of demotions
