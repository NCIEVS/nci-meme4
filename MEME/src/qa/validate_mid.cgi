...
# 01/09/2007 JFW (1-D5AC1): Wrap queries in description at 80 characters 
...
#
# Version info:
# 07/15/2004 4.1.1: hash/sort_area_size => 200000000
# 12/09/2003 4.1.0: descriptions now in a separate state
# 10/09/2003 3.5.0: Added auto_fix to allow user to fix problems from web
#                   Released
# 04/01/2003 3.4.2: change database
# 02/20/2003 3:4.1: document.location -> window.location.href
# 09/09/2002 3.4.0: When an individual check is run, it shows
#                   the description and adjustment info
# 08/27/2002 3.3.0: Uses new make_checklist script.  Allows
#                   user to run all sections or one section at a time.
# 07/17/2002 3.2.0: Upgraded to keep track of db if set in URL
# 01/30/2002 3.1.0: Released
#                   midsvcs names were changed from things like
#                   current-editing-tns to editing-db
# 11/12/2001 3.0.7: Fixed a bug involving quote characters in the
#                   check names (caused ie5.0 to fail)
# 08/24/2001 3.0.6: The application was upgraded to support the creation
#                  of checklists for queries that return concept ids
# 03/20/2001 3.0.5: First version
#
$release = "4";
$version = "1.1";
$version_authority = "BAC";
$version_date = "07/15/2004";
...
%states = (
	     ...
	     "INDEX" => 
	         ["PrintHeader","PrintINDEX","PrintFooter","MID Validation - Index Page","Validate Mid"],
	     "RUNNING_SCRIPT" => 
...
#
sub PrintINDEX {
    $log_name = `ls -t Logs/validate.*.log | head -1`;
    chomp($log_name);
    $log_name =~ s/Logs\///;
    ...
    if($command eq "RESUME") {
       $dbh->do("UPDATE system_status SET status='ON' WHERE system='mid_validation'")
       ||
    ((print "<span id=red>Error updating system_status ($DBI::errstr).</span>")
       ...
       $dbh->do("UPDATE system_status SET status='OFF' WHERE system='mid_validation'")
       ||
    ((print "<span id=red>Error updating system_status ($DBI::errstr).</span>")
             &&  return);
       $command = "RESUME";
    ...
    $sh = $dbh->prepare(qq{
    SELECT status FROM system_status WHERE system='mid_validation'
    }) ||
...
<ul>
    ...
    $sh = $dbh->prepare(qq{
    SELECT DISTINCT check_type FROM mid_validation_queries
    }) || 
    ((print "<span id=red>Error preparing query ($DBI::errstr).</span>")  
             &&  return);
...
<tr>
<td colspan="2"><b><a href="$cgi?state=EDIT_INDEX&db=$db" onMouseOver='window.status="Edit Validate Mid Checks"; return true;' onMouseOut='window.status=""; return true;'>Edit checks</a></b></td>
</tr>
...
sub PrintRUNNING_SCRIPT {
	...
	system qq{$ENV{MEME_HOME}/bin/validate.pl $db mid "$check_type" > $inc/Logs/$log_name & };
	if ($?) {
...
#	print "$ENV{ORACLE_HOME} $! $?<br>\n";
	`chmod 666 Logs/$log_name`;
	sleep 1;
    };

    open (F,"Logs/$log_name") || die "couldn't open file: $log_name\n";
    @lines = <F>;
    close(F);
    $finished=0;
    ...
    if ($error) {
	print qq{ <span id=red>There was an error running the Validate MID script.  The log that was produced appears below:</span><p><pre>
};
	print join("/n",@lines),"/n";
	print qq{</pre>};
    } elsif ($finished){
	print qq{ Validate MID script is done.
	      Click <a href="$cgi?state=REVIEW_LOG&db=$db&log_name=$log_name"> here</a> to see the log.
	      };
    } else {
	print qq { The Validate MID Script is running. The resulting log is displayed below.  This page will refresh itself every 10 seconds.  The duration of the process is between 30 minutes to 1 hour.  
...
sub PrintREVIEW_LOGS {
    print qq{
<i>The following is a list of all validate MID logs:</i>
<br>&nbsp;
<blockquote>
<form action="$cgi" method="GET">
...
sub PrintLIST_DATA {
    ...
    if ($make_checklist eq "Y") {
      $enc_name = &meme_utils::escape($check_name);
      $mc_text = qq{
        This MID validation check can be made into a checklist.
...
sub PrintMAKE_CHECKLIST{
    ...
    $output = `$checklist_script -d $db -c $checklist_name -b validate_mid -t AH  < /tmp/t.$$`;
    if ($? != 0) {
...
sub PrintEDIT_FORM {
	...
	$sh = $dbh->prepare(qq{
	    ...
	    FROM mid_validation_queries WHERE check_name=?
	    }) || 
		((print "<span id=red>Error preparing query ($query,$DBI::errstr).</span>")  
		 &&  return);
...
sub PrintEDIT_COMPLETE {
  ...
  $auto_fix =~ s/\r/ /g;
	...
	SET check_name = ?
      ...
      # insert a row
      $row_count = $dbh->do( qq{
        INSERT INTO mid_validation_queries
     ...
     # delete row
      $row_count = $dbh->do( qq{
        DELETE FROM mid_validation_queries WHERE check_name= ? },
        {"dummy"=>1},$check_name);
	...
	Click <a href="$cgi?state=EDIT_INDEX&db=$db">here</a> to return to the index page.
     ...
     # Update code here
      ...
      #update row
      $row_count = $dbh->do( qq{
        UPDATE mid_validation_queries SET
            check_name = ?,
            check_type = ?,
            query = ?,
            description = ? ,
...
CGI arguments in the standard form "key=value&key=value...". 
 ...
 Parmaters:
  ...
  log_name      :  Parameter to states that review logs.  It should be
                   the name of a file in the Logs subdirectory of the
