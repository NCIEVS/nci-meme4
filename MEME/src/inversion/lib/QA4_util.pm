...
open(LABELS, "$ENV{INV_HOME_OLD}/etc/$file.fields") or die "Can't open $file.fields, $!";
while(<LABELS>){
	chomp;
	($fnum,$name)=split(/\|/);
	$fnum2name{$fnum}=$name;
...
$char_ent = "$ENV{INV_HOME_OLD}/etc/char_entities.txt";
...
sub ck_field_lens{
    ...
    if($file =~ /class/){
      @len = (12,20,40,100,1,1,1,1200,10);
     }
    elsif($file =~ /relat/){
