...
proc prepareData {} {
    ...
    set count 0;
    # now gather SEMANTIC types for saids (for now) from attributes.
    set inf [open "$g_srcDir/attributes.src" "r"]
    while {![eof $inf]} {
	gets $inf line
	set lst [split $line "|"]
	set sgid [lindex $lst 1]
	set name [lindex $lst 3]
	set val [lindex $lst 4]
	set sgt [lindex $lst 10]
	set sgq [lindex $lst 11]

	if {$name == "SEMANTIC_TYPE"} {
	    if {$sgt == "SOURCE_ATOM_ID" || $sgt == "SRC_ATOM_ID"} {
		incr count
		lappend tqi2sty(SAID|$sgid) $val
	    } else {
		lappend tqi2sty($dgt|$sgq|$sgid) = $val
	    }
	}
    }
    close $inf
    puts "$count STYs encountered"
	...
	set tg [lindex $lst 2]
	set tty [lindex [split $tg "/"] 1]
	set code [lindex $lst 3]
	set str [lindex $lst 7]
	set scui [lindex $lst 10]
	set sdui [lindex $lst 11]
	
	set dsty ""
	if {[info exists g_defs($num)]} {
	    # find its sty if any.
	    if {[info exists tqi2sty(SAID|$num)]} {
		set dsty $tqi2sty(SAID|$num);
	    } elseif {[info exist tqi2sty(CODE_TERMGROUP|$tg|$code)]} {
		set dsty $tqi2sty(CODE_TERMGROUP|$tg|$code);
	    } elseif {[info exists tqi2sty(SOURCE_CUI||$scui)]} {
		set dsty $tqi2sty(SOUCE_CUI||$scui)
	    }

	    set g_defs($num) "$code|($tty) $str <$num> $dsty"
	}
    }
    close $irf
...
proc cb_printData {} {
    puts "selected PrintDat"
    global base
    iwidgets::canvasprintbox .cpb -stretch 0 -filename "page.out" \
	-orient portrait -output file -pagesize Letter
    .cpb setcanvas $base.trfr.h
    .cpb print
