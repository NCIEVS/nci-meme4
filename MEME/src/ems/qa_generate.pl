...
# Generates contents for a QA bin
sub do_qa_generate {
  my($html);
  ...
  if ($bin_name eq "__ALL__") {
    my(@bins) = EMSUtils->getBinNames($dbh, "QA");
    foreach $bin_name (@bins) {
      EMSBinlock->unlock($dbh, {bin_name=>$bin_name});
      $query->param(-name=>'bin_name', -value=>$bin_name);
      &do_qa_generate;
    }
    &printhtml({redirect=>$query->url() . "?$DBget&action=qa_bins&order_by=$order_by"}) unless $query->param('batch');
    ...
    EMSUtils->bin_generate($dbh, $bin_name, 'QA');
    EMSBinlock->unlock($dbh, {bin_name=>$bin_name});

    if ($query->param('batch')) {
      print "Bin: $bin_name regenerated", "\n";
    } else {
      &printhtml({redirect=>$query->url() . "?$DBget&action=qa_bins&bin_name=$bin_name&order_by=$order_by"});
    }
  }
  return;
}
