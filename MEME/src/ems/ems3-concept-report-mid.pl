...
%compat_actionmap =
  (
   ...
   searchbycui=>{action=>'search', subaction=>'cui'},
   searchbycode=>{action=>'search', subaction=>'code'},
   searchbysourcerowid=>{action=>'search', subaction=>'source_row_id'},
   searchbynormstr=>{action=>'search', subaction=>'normstr'},
   searchbynormwrd=>{action=>'search', subaction=>'normwrd'},
...
$dbh->executeStmt("alter session set NLS_DATE_FORMAT='dd-mm-yyyy hh24:mi:ss'");
...
sub leader {
  ...
  $html .= join("\n",
                ...
                &emsinfo,
                $query->p, $query->hr,
                $query->start_form(-action=>'showattributes.pl', -method=>'POST', -target=>'_new'),
                $cgiPOST,
                $query->hidden(-name=>'action', -value=>'searchbyconceptid', -override=>1),
                &showAttributes,
                $h,
                $query->submit({-value=>'Get All Attributes'}),
                $query->p, $query->hr,
                &helplink,
                $query->p, $query->hr
                );
...
sub showAttributes {
  my($html);
  $html .= "Click here to view all the attributes of this concept ";
}
